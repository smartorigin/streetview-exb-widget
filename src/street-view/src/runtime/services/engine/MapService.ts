...
class MapService {
   ...
   */
  onClick(
    callback: (e: __esri.ViewClickEvent) => void,
    options?: { skipOnFeatureHit?: boolean }
  ) {
    return this.jmv.view.on('click', async (e) => {
      if (options?.skipOnFeatureHit) {
        const hitTestResult = await this.jmv.view.hitTest(e);
        const hasFeature = hitTestResult.results.some(
          (result) => result.layer?.type === 'feature'
        );
  ...
  public async setPopupActions() {
    ...
    for (let layerViewId in this.jmv.jimuLayerViews) {
      let layerView = this.jmv.jimuLayerViews[layerViewId];
      if (layerView.layer && layerView.layer.popupTemplate) {
        layerView.layer.popupTemplate.actions ||= [];
        if (
          layerView.layer.popupTemplate.actions.find(
            (f) => f.id === this.POPUP_STREETVIEW_ACTION_ID
          ) === undefined
        ) {
          layerView.layer.popupTemplate.actions.push({
            ...
            //not standard
            layerUrl: layerView.layer.parsedUrl
              ? layerView.layer.parsedUrl.path
              : layerView.layer.url
          });
        }
      }
